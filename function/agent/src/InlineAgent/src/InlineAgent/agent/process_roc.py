        ...
        for invocationInput in roc_event["invocationInputs"]:
            ...
            if (
                ...
                if actionInvocationType == "USER_CONFIRMATION_AND_RESULT":
                    ...
                    # Add a small delay between tool invocations to prevent rapid API calls
                    if len(inlineSessionState["returnControlInvocationResults"]) > 0:
                        import time
                        time.sleep(0.5)  # 500ms delay between tool calls
